...
class ILC_OnlyPinocchioFishing():
    ...
    def __init__(self, 
        ...
        # self.k_ii = k_ii + self.delta_k_ii #- self.delta_k_ii up to 6
        ...
        # self.directory = 'ILC_DATA_6_2'
        # self.directory = 'ILC_DATA_8_{}_{}'.format(dt,max_step)
        ...
        self.max_iter = 500
        self.toll = 0.017 # 0.048 # 0.065 # 5e-3 # 1e-2 0
        self.r = r
        self.conta_incr = 0
        self.sign_u = 1
        self.Gamma_fb = 0.1
        self.Gamma_ff = -0.1
        # ff gains 
        self.gamma0 = self.sign_u * 1
        self.gamma1 = self.sign_u * 0.01
        self.gamma2 = self.sign_u * 0.0001
        # fb gains frank 
        self.K_P = self.sign_u * 1 # 15
        self.K_V = self.sign_u * 0.01
    ...
    def saveData(self, iteration, error, pos_x, vel_x, acc_x, action_ff, action_fb, u_per_iter_ff, q, q_dot):
        self.dir_plot = './' + self.directory + '/Iter_{}/'.format(iteration)
    ...
    def savePlot(self, iteration):
        dir_plot = './' + self.directory + '/Iter_{}/'.format(iteration)
        ...
        if self.task == 'Joint':
            ...
            # y_dot = vel_x
            # y_ddot = acc_x
            label_vel_str = r'Vel Cartesian $[X]$'
            label_acc_str = r'Acc Cartesian $[X]$'
            y_label_y_des = r'Tip Pos $X$ $[m]$'
        plt.figure(clear=True)
        plt.ylabel(r'Action $[Nm]$')
        plt.xlabel(r'Time $[s]$')
        plt.plot(self.time, action_ff, color='red', label='ff')
        plt.plot(self.time, action_fb, color='green', label='fb')
        plt.plot(self.time, action, color='blue', label='ff + fb')
        plt.grid()
        ...
        plt.xlabel(r'Time $[s]$')
        plt.plot(self.time, u_per_iter_ff, color='red', label='ff')
        plt.grid()
        plt.legend(loc='best', shadow=True, fontsize='10')
        ...
        plt.xlabel(r'Time $[s]$')
        plt.plot(self.time, self.y_dot_vett, color='green', label=label_vel_str)
        plt.plot(self.time, self.y_ddot_vett, color='blue', label=label_acc_str)
        plt.grid()
        plt.legend(loc='best', shadow=True, fontsize='10')
        ...
        plt.xlabel(r'Time $[s]$')
        plt.plot(self.time, self.y_des,color='blue',label='Des')
        plt.plot(self.time, self.y_vett, color='red',label='Real')
        plt.grid()
        plt.legend(loc='best', shadow=True, fontsize='10')
        ...
        plt.xlabel(r'Time $[s]$')
        plt.plot(self.time, err, color='red')
        plt.grid()
    ...
    def getNewControlEachTime(self, y, y_dot, y_ddot, u_old, i):
        ...
        # u_new_ff =  self.Gamma_ff * err # classic ## finish to test tonight 
    ...
    def main(self):
        ...
        for j in range(0, self.max_iter): # iteration
            ...
            self.y_ddot_vett = []
            self.control_action_vett = []
            # print('Init Pos: {}\nInit Vel: {}\nInit Acc: {}'.format(self.data.pos,self.data.vel,self.data.acc))
            # conta_reached = 0
            for i in range(0, self.steps): # time 
                ...
                # u_fb = np.sign(u_fb) * self.Gamma_fb * u_fb
                u_fb = self.Gamma_fb * u_fb
                control_action = action_plug + u_fb
                # print('action_plug: {}\nu_fb: {}'.format(action_plug, u_fb))
                ...
                self.q_vett.append(q)
                self.q_dot_vett.append(q_dot)
                self.q_ddot_vett.append(self.data.xout)
                self.pos_vett.append(self.data.pos[0])
                self.vel_vett.append(self.data.vel[0])
                self.acc_vett.append(self.data.acc[0])
                ...
                ## This has sense only for for regularization
                # if abs(y - target_pos[0]) < self.toll*10:
                #     conta_reached = conta_reached + 1
                #     print('=================================================================')
                #     print('Target Reached!\nTime: {}s\nN: {}'.format(i * self.dt, conta_reached)) 
                #     if conta_reached > 10:
                #         print('FINISH!\nRobot reaches to the target for enough time!')
                ...
                #         self.u_new[h, i], self.err[h, i] = self.getNewControlEachTime(y[h], y_dot[h], y_ddot[h], action_plug[h], i, j)
                # if j == 0:
                #     self.u_new[i], self.err[i] = self.getNewControlEachTime(y, y_dot, y_ddot, action_plug, i)
            ...
            print('Final Err: {}'.format(self.err_t_f[j]))
            print('Error RMS: {}'.format(self.err_rms[j])) 
            if j > 1:
            ...
            # if j > 3:
            #     if self.conta_incr > 3:
            #         print(colored('======== The Learning Gain \GAMMA has changed sign ... ==========','yellow'))
            #         self.Gamma_ff = - self.Gamma_ff
            #         self.conta = 0
            #         for i in range(0,self.steps):
            #             self.u_new[i], self.err[i], self.u_per_iter_ff[i] = self.getNewControlEachTime(self.pos_vett[i], self.vel_vett[i], self.acc_vett[i], action[i], i)
            #             # self.u_new[i] = self.u_new[i] + self.u_fb_vett[i] # iterative ff + fb
            #     else:
            #         for i in range(0,self.steps):
            #             self.u_new[i], self.err[i], self.u_per_iter_ff[i] = self.getNewControlEachTime(self.pos_vett[i], self.vel_vett[i], self.acc_vett[i], action[i], i)
            #             # self.u_new[i] = self.u_new[i] + self.u_fb_vett[i] # iterative ff + fb
            ...
            if self.err_rms[j] < self.toll: # and self.err_t_f[j] < self.toll:
                print('FINISH')
                self.FINISH_LEARNING = True
        ...
        if self.FINISH_LEARNING:
            print('LEARNING HAS BEEN SUCCESSUFULLY ACHIVED!')
        ...
        plt.xlabel(r'Time $[s]$')
        plt.plot(self.time, self.y_des, color='red', linestyle='-',linewidth=2)
        plt.plot(self.time, self.pos_vett, color='blue', linestyle='-.',linewidth=2)
        plt.grid()
